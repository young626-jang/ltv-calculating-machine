...
# 버튼 아래에 시세 입력창
raw_price = st.text_input("KB 시세 (만)", "0")
...
text_to_copy += f"{type_of_price} | KB시세: {raw_price} | 방공제 금액: {deduction:,}만\n"
...
# 결과 출력
st.write(f"컨설팅 비용: {int(컨설팅_fee):,}만")
st.write(f"브릿지 비용: {int(bridge_fee):,}만")
st.write(f"🔗 총 비용: {int(총_fee):,}만")
